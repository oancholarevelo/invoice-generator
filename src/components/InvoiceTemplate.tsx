...
const InvoiceTemplate = React.forwardRef<HTMLDivElement, InvoiceTemplateProps>(({ data, currency }, ref) => {
  ...
  return (
    ...
    <div ref={ref} className="flex flex-col p-4 sm:p-10 bg-white dark:bg-neutral-800 w-full min-h-full">
      ...
      {/* ADDED: A new div that grows to push the footer down */}
      <div className="flex-grow">
        <div className="flex justify-between">
          ...
          <div className="text-end">
            ...
            <span className="mt-1 block text-gray-500 dark:text-neutral-500"># {data.invoiceNumber}</span>
            <address className="mt-4 not-italic text-gray-800 dark:text-neutral-200" dangerouslySetInnerHTML={{ __html: data.company.address.replace(/\n/g, '<br />') }} />
          </div>
        </div>

        <div className="mt-8 grid sm:grid-cols-2 gap-3">
          <div>
            <h3 className="text-base font-semibold text-gray-800 dark:text-neutral-200">Bill to:</h3>
            <h3 className="text-base font-semibold text-gray-800 dark:text-neutral-200">{data.client.name}</h3>
            <address className="mt-2 not-italic text-gray-500 dark:text-neutral-500" dangerouslySetInnerHTML={{ __html: data.client.address.replace(/\n/g, '<br />') }} />
          </div>
          <div className="sm:text-end space-y-2">
            <dl className="grid sm:grid-cols-5 gap-x-3">
              <dt className="col-span-3 font-semibold text-gray-800 dark:text-neutral-200">Invoice date:</dt>
              <dd className="col-span-2 text-gray-500 dark:text-neutral-500">{data.invoiceDate}</dd>
            </dl>
            <dl className="grid sm:grid-cols-5 gap-x-3">
              <dt className="col-span-3 font-semibold text-gray-800 dark:text-neutral-200">Due date:</dt>
              <dd className="col-span-2 text-gray-500 dark:text-neutral-500">{data.dueDate}</dd>
            </dl>
          </div>
        </div>
      ...
      {/* END of the growing content wrapper */}


      {/* This is the footer section */}
      <div className="mt-8 sm:mt-12 pt-8 border-t border-gray-200 dark:border-neutral-700">
        <h4 className="text-lg font-semibold text-gray-800 dark:text-neutral-200">Thank you!</h4>
        <p className="text-gray-500 dark:text-neutral-500">{data.notes}</p>
        <div className="mt-2">
